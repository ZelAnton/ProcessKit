...
- **`GetFullOutputAsyncCore`** builds a `ProcessSession` with a `TextBufferSink` for each stream → **faithful** decoded text (exact line endings + trailing newline; no line-rejoin).
- **`GetBytesOutputAsyncCore`** builds a `ProcessSession` with a `ByteBufferStdOutSink` (raw stdout bytes) + a `TextBufferSink` for stderr (faithful stderr text). The bytes path is no longer a separate lifecycle — the duplication that previously existed is gone.

Stdout transport is the only thing that varies, behind two small internal seams:

- **`IStdOutSink`** ([IStdOutSink.cs](src/ProcessKit/IStdOutSink.cs)) — per-stream transport (used for stdout **and** stderr); `PumpAsync(StreamReader, handler, ct)`: `LineChannelStdOutSink` (decoded lines → `ILineBuffer`, tracks `LineCount`, tees per line — backs streaming), `ByteBufferStdOutSink` (raw `BaseStream` → `MemoryStream`, exact bytes), `TextBufferSink` (faithful decoded text — exact endings/trailing; with a handler it also tees per line via a line-splitter matching `ReadLine` semantics). `ProcessSession` takes a stdout sink and an **optional** stderr sink; when the stderr sink is null it keeps a line buffer exposed via `StdErr`/`StdErrLineCount` (the streaming `RunningProcess` path). The bulk text/byte cores pass a `TextBufferSink` for stderr.
...
`GetBytesOutputAsync` is the only extension that **does not** surface an `IRunningProcess` — the handle is line-oriented (channels of `string`), and forcing binary stdout through string decoding would be lossy. It builds a `ProcessSession` with a `ByteBufferStdOutSink` (raw `BaseStream` → `MemoryStream`) for stdout and a `TextBufferSink` for stderr (faithful text). `GetFullOutputAsync` is built the same way (a `TextBufferSink` per stream) so its captured text is byte-faithful. Both await `session.StdOutPumpCompletion` **and** `StdErrPumpCompletion` before reading the captured output (`Completion` can resolve before the pipes are fully drained), and both bypass `runner.Start` so they apply `ProcessRunner.Defaults` explicitly. They share the full lifecycle with `RunningProcess` — no duplicated group/timeout/exit logic. (`ToResultAsync`, by contrast, reads the line-oriented handle and is therefore line-normalized — documented on the method.)
