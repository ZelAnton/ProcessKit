...
- `IRunningProcess` is **line-oriented** by design (`IAsyncEnumerable<string>` for both stdout and stderr). Raw byte access is intentionally absent from the interface; `GetBytesOutputAsync` runs the same `ProcessSession` lifecycle with a `ByteBufferStdOutSink` (raw `BaseStream` → `MemoryStream`) instead of surfacing a handle. Do not add a raw `Stream` property to `IRunningProcess` without explicit approval.
